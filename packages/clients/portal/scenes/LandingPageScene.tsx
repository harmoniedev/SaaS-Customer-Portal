...
export const LandingPageScene = ({ slug }: LandingPageSceneProps) => {
  ...
  const [noUrl, setNoUrl] = useState<string>(''); // variables provided in sanity
  const [yesUrl, setYesUrl] = useState<string>(''); // variables provided in sanity
  ...
  const handleLoad = () => {
    ...
    if (frame && isIFrame(frame)) {
      frame.contentWindow.postMessage(
        process.env.IFRAME_KEY,
        process.env.BASE_IFRAME_API,
      );
      setIsLoading(false);
    }
  };
    ...
    if (typeof window !== undefined) {
      window.addEventListener('message', (event) => {
        if (
          ...
          event.origin.startsWith(`${process.env.BASE_IFRAME_API}`)
        ) {
  ...
  return (
    <div>
      ...
      <Dialog
        mode="form"
        onOpenChange={(value) => setIsConsent(value)}
        open={isConsent}
      >
        <div className="lg:max-w-lg shadow-xl bg-white border border-black border-opacity-5 px-8 py-4 lg:px-16 lg:py-10">
          ...
          <p className="mt-4">
            Some question placeholder goes here Some question placeholder goes here
            Some question placeholder goes here Some question placeholder goes here
            Some question placeholder goes here Some question placeholder goes here
          </p>
          <div className="mt-8 flex space-x-4 justify-end">
            <Button label="Reject all" theme="white" href={noUrl} />
            <Button label="Accept all" href={yesUrl} />
          </div>
        </div>
      </Dialog>
      <iframe
