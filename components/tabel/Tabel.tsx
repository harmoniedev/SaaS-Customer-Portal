...
}: TabelProps) =>
{
  ...
  {
    setIsSelectedAll(
      checkedUsersList.length > 0 && usersList.length > 0,
    );
  ...
  const finalListAllUsers = listAllUsers.map(({ email,product_name, build_version, last_date, first_date,  }) => ({
    "USERS":email,"BASE DOMAIN":email.split("@")[1],"PRODUCT NAME":product_name, "BUILD VERSION":build_version,"FIRST ACCESS": !!last_date && formatDate(last_date), "LAST ACCESS": !!first_date && formatDate(first_date), 
  }));
    ...
    <div className="pb-20">
      <div className="flex flex-col justify-between gap-3 mb-3">
        <div className="flex gap-4 text-indigo-500 mb-4 md:mb-0 items-center">
          <Title size="xs">Registered Users</Title>
        ...
        <div className="flex flex-rows gap-3 lg:gap-3 md:justify-end">
          <div className='mr-auto'>
            ...
            <Button
              label={isSelectedAll ? 'Deselect All Users' : 'Select All Users'}
              align="center"
              size="md"
          ...
          <div className="relative">
            <Tooltip text={"Export all users"}>
              ...
              <CSVLink
                ...
                data={finalListAllUsers}
                filename={`users-${email_domain}-${moment().format('YYYYMMDD-hhmmss')}.csv`}
                target="_blank" />
