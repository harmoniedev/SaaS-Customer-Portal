...
}: TabelProps) =>
{
  ...
  const finalListAllUsers = listAllUsers.map(({ email, build_version, last_date, first_date, product_name }) => ({
    email, build_version, last_date, first_date, product_name 
  }));
  ...
  return (
    <div className="pb-20">
        ...
        <div className="flex flex-rows gap-3 lg:gap-3 md:justify-end">
          ...
          <Tooltip text={"Export all users"}>
            ...
            <CSVLink
              ...
              filename={`users-harmonie-${moment().format('YYYYMMDD-hhmmss')}.csv`}
              target="_blank"/>
      ...
      <button onClick={() => setPerPage(50)}>Read More</button>
    </div>
  );
};
